...
import { ServerCog, Zap, CheckCircle2, Workflow } from "lucide-react";
      ...
      <div style={{ width: '100%', maxWidth: '1200px', margin: '0 auto', padding: '120px 20px 60px 20px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: '60px', alignItems: 'center', marginBottom: '80px' }}>
          <div className="fadeUp">
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: 'rgba(6, 182, 212, 0.1)', padding: '6px 12px', borderRadius: '20px', color: '#06B6D4', fontSize: '0.9rem', fontWeight: 600, marginBottom: '20px', border: '1px solid rgba(6, 182, 212, 0.2)' }}>
              <ServerCog size={16} /> Services
            </div>
            <h1 style={{ fontSize: '3.5rem', fontWeight: 700, marginBottom: '20px', lineHeight: 1.1, background: 'linear-gradient(to right, #F8FAFC, #9CA3AF)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
              Backend Engine Creation
            </h1>
            <p style={{ fontSize: '1.2rem', color: '#9CA3AF', lineHeight: 1.6, marginBottom: '40px' }}>
              Design and deployment of highly scalable, distributed backend engines built to power global digital ecosystems seamlessly.
            </p>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '30px', marginBottom: '80px' }}>
          <div className="fadeUp" style={{ animationDelay: '0.3s', background: 'rgba(255, 255, 255, 0.02)', border: '1px solid rgba(255, 255, 255, 0.05)', borderRadius: '20px', padding: '40px', backdropFilter: 'blur(10px)' }}>
            ...
            <h2 style={{ fontSize: '1.5rem', marginBottom: '16px', color: '#F8FAFC' }}>System Challenges</h2>
            <ul style={{ listStyle: 'none', padding: 0, margin: 0, color: '#9CA3AF' }}>
              {['High latency under load', 'Inflexible monolithic structures', 'Unreliable data synchronization', 'Poor observability & monitoring', 'Costly infrastructure scaling'].map((item, i) => (
          ...
          <div className="fadeUp" style={{ animationDelay: '0.4s', background: 'rgba(255, 255, 255, 0.02)', border: '1px solid rgba(255, 255, 255, 0.05)', borderRadius: '20px', padding: '40px', backdropFilter: 'blur(10px)' }}>
            <Workflow color="#06B6D4" size={32} style={{ marginBottom: '20px' }} />
            <h2 style={{ fontSize: '1.5rem', marginBottom: '16px', color: '#F8FAFC' }}>How We Solve This</h2>
            <p style={{ color: '#9CA3AF', lineHeight: 1.6 }}>
              We architect high-performance, fault-tolerant backend foundations. Utilizing microservices, serverless patterns, and advanced data caching strategies to guarantee zero downtime and infinite scalability.
            </p>
          </div>
        </div>
        ...
        <div className="fadeUp" style={{ animationDelay: '0.5s', marginBottom: '80px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' }}>
            {[
              { title: 'Microservices Topologies', desc: 'Decoupled, containerized service architectures preventing single points of failure.', icon: <CheckCircle2 color="#06B6D4" size={24} /> },
              { title: 'High-Throughput APIs', desc: 'GraphQL and gRPC endpoints capable of handling tens of thousands of requests per second.', icon: <CheckCircle2 color="#06B6D4" size={24} /> },
              { title: 'Distributed Memory Caches', desc: 'Redis and Memcached integrations drastically reducing database read latencies.', icon: <CheckCircle2 color="#06B6D4" size={24} /> },
              { title: 'Event-Driven Streaming', desc: 'Kafka and RabbitMQ integrations for asynchronous processing of heavy data workloads.', icon: <CheckCircle2 color="#06B6D4" size={24} /> },
              { title: 'Database Sharding', desc: 'Horizontal scaling techniques partitioning massive SQL/NoSQL databases across clusters.', icon: <CheckCircle2 color="#06B6D4" size={24} /> },
              { title: 'Zero-Downtime CI/CD', desc: 'Automated blue/green deployment pipelines ensuring updates never impact user experience.', icon: <CheckCircle2 color="#06B6D4" size={24} /> }
            ].map((feature, i) => (
              <div key={i} style={{ background: 'rgba(255, 255, 255, 0.02)', border: '1px solid rgba(255, 255, 255, 0.05)', borderRadius: '16px', padding: '24px', transition: 'all 0.3s ease', cursor: 'default' }} className="serviceCard">
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '12px' }}>
                  <div style={{ background: 'rgba(6, 182, 212, 0.1)', padding: '10px', borderRadius: '12px', display: 'flex' }}>
                    {feature.icon}
                  </div>
