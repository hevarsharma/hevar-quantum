...
import { Database, Zap, CheckCircle2, Workflow } from "lucide-react";
      ...
      <div style={{ width: '100%', maxWidth: '1200px', margin: '0 auto', padding: '120px 20px 60px 20px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: '60px', alignItems: 'center', marginBottom: '80px' }}>
          <div className="fadeUp">
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: 'rgba(239, 68, 68, 0.1)', padding: '6px 12px', borderRadius: '20px', color: '#EF4444', fontSize: '0.9rem', fontWeight: 600, marginBottom: '20px', border: '1px solid rgba(239, 68, 68, 0.2)' }}>
              <Database size={16} /> Services
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '30px', marginBottom: '80px' }}>
          <div className="fadeUp" style={{ animationDelay: '0.3s', background: 'rgba(255, 255, 255, 0.02)', border: '1px solid rgba(255, 255, 255, 0.05)', borderRadius: '20px', padding: '40px', backdropFilter: 'blur(10px)' }}>
            ...
            <h2 style={{ fontSize: '1.5rem', marginBottom: '16px', color: '#F8FAFC' }}>Data Challenges</h2>
            <ul style={{ listStyle: 'none', padding: 0, margin: 0, color: '#9CA3AF' }}>
              {['Frequent IP blocking & captchas', 'Unstructured and messy source data', 'Inability to scale extraction rates', 'Changing DOM structures breaking scrapers', 'Ensuring data compliance and ethics'].map((item, i) => (
          ...
          <div className="fadeUp" style={{ animationDelay: '0.4s', background: 'rgba(255, 255, 255, 0.02)', border: '1px solid rgba(255, 255, 255, 0.05)', borderRadius: '20px', padding: '40px', backdropFilter: 'blur(10px)' }}>
            <Workflow color="#EF4444" size={32} style={{ marginBottom: '20px' }} />
        ...
        <div className="fadeUp" style={{ animationDelay: '0.5s', marginBottom: '80px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' }}>
            {[
              { title: 'Headless Browser Arrays', desc: 'Distributed clusters of Puppeteer instances parsing complex single page JavaScript applications.', icon: <CheckCircle2 color="#EF4444" size={24} /> },
              { title: 'Dynamic Proxy Rotation', desc: 'Intelligent multiplexing across millions of residential IPs to entirely avoid rate-limiting.', icon: <CheckCircle2 color="#EF4444" size={24} /> },
              { title: 'Anti-Bot Bypass', desc: 'Automated heuristic solvers to pass Cloudflare checks and complex image/audio captchas.', icon: <CheckCircle2 color="#EF4444" size={24} /> },
              { title: 'Machine Learning Parsers', desc: 'Computer vision models identifying key semantic structures even when page DOMs mutate.', icon: <CheckCircle2 color="#EF4444" size={24} /> },
              { title: 'Real-Time ETL Pipelines', desc: 'Instantaneous data cleaning and normalization before bulk insertion to data lakes.', icon: <CheckCircle2 color="#EF4444" size={24} /> },
              { title: 'API Ingestion', desc: 'GraphQL endpoints serving the curated dataset back to your internal application logic.', icon: <CheckCircle2 color="#EF4444" size={24} /> }
            ].map((feature, i) => (
              <div key={i} style={{ background: 'rgba(255, 255, 255, 0.02)', border: '1px solid rgba(255, 255, 255, 0.05)', borderRadius: '16px', padding: '24px', transition: 'all 0.3s ease', cursor: 'default' }} className="serviceCard">
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '12px' }}>
                  <div style={{ background: 'rgba(239, 68, 68, 0.1)', padding: '10px', borderRadius: '12px', display: 'flex' }}>
                    {feature.icon}
                  </div>
